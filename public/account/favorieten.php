...
$recepten = $database->select(
    "SELECT r.`ReceptID`, `Title`, `Personen`, `Moeilijkheid`, `Berijdingstijd`, `Foto`, m.`Maaltijdtype`, `Date` FROM favoriet f LEFT JOIN recept r ON f.ReceptID = r.ReceptID LEFT JOIN `maaltijdtype` m ON r.`MaaltijdtypeID` = m.`MaaltijdtypeID`WHERE f.GebruikerID = ?",
    ["i", [$user]]
);
